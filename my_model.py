...
class Layer(object):
    ...
    def build_layer(self):
        ...
        if not self.is_layer_built:
            ...
            # register ops
            if self.design['type'] in {'default'}:
...
class SequentialNet(object):
    ...
    def __call__(self, net_input, is_training=True):
        net_output = net_input
        for layer in self.layers:
            net_output = layer.apply(net_output)

        return net_output

    def apply(self, net_input, is_training=True):
        self.__call__(net_input, is_training)
